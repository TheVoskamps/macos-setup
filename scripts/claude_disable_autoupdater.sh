...
#
#   1. `claude config set -g autoUpdates false` — the CLI's own setting.
#   2. `export DISABLE_AUTOUPDATER=1` in ~/.zshrc — the environment
#      override, which also covers invocations that bypass the CLI config.
#
...
# Lever 1 used to be a bare `claude config set ...` line in the Makefile's
# per-slot recipe; it moved in here when post-install dispatch became a
# `[profile] post_install` list of scripts, so both levers pull together.

set -euo pipefail

# Lever 1. Best-effort: the `claude` CLI may not be installed yet (the cask
# is installed by the same tier that runs this script, and a `brew bundle`
# failure would leave it absent), and the setting is not load-bearing enough
